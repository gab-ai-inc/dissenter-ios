...
class ShareToDissenterViewController: SLComposeServiceViewController {
    ...
    override func configurationItems() -> [Any]! {
        ...
        provider.loadItem(of: provider.isUrl ? kUTTypeURL : kUTTypeText) { item, error in
            ...
            if let dissenterUrl = urlItem.absoluteString.addingPercentEncoding(withAllowedCharacters: .alphanumerics).flatMap(self.urlScheme) {
                self.handleUrl(dissenterUrl)
            }
        }
